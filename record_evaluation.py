import os
import gymnasium as gym
import ale_py
import numpy as np
import pandas as pd
import argparse

from gymnasium.wrappers import RecordVideo
from stable_baselines3.dqn.dqn import DQN as SB3_DQN

gym.register_envs(ale_py)

# =========================
# ARGUMENTOS
# =========================

parser = argparse.ArgumentParser()
parser.add_argument("--model", type=str, required=True,
                    help="Path al modelo (con o sin .zip)")
parser.add_argument("--episodes", type=int, default=20,
                    help="Número de episodios de evaluación")
args = parser.parse_args()

MODEL_PATH = args.model
EPISODES = args.episodes

# =========================
# CUSTOM DQN (para compatibilidad)
# =========================

class CustomDQN(SB3_DQN):
    def __init__(self, *args, loss_type="huber", double_q=False, **kwargs):
        super().__init__(*args, **kwargs)
        self.loss_type = loss_type
        self.double_q = double_q

# =========================
# CARGAR MODELO
# =========================

if not MODEL_PATH.endswith(".zip"):
    MODEL_PATH += ".zip"

model = CustomDQN.load(MODEL_PATH, device="cuda")

# =========================
# IDENTIFICAR EXPERIMENTO
# =========================

exp_name = os.path.basename(os.path.dirname(MODEL_PATH))
log_dir = os.path.join("logs", exp_name)

print("\n==============================")
print("Evaluating:", exp_name)
print("==============================")

# =========================
# MÉTRICAS DE TRAINING
# =========================

monitor_path = os.path.join(log_dir, "monitor.csv")

if os.path.exists(monitor_path):
    df = pd.read_csv(monitor_path, skiprows=1)

    global_mean = df["r"].mean()
    global_std = df["r"].std()
    best_score = df["r"].max()

    print("\n===== TRAINING METRICS =====")
    print(f"Media Global: {global_mean:.2f} ± {global_std:.2f}")
    print(f"Mejor Score: {best_score:.2f}")
else:
    print("\nNo monitor.csv encontrado")

# =========================
# TIEMPO DE ENTRENAMIENTO
# =========================

time_path = os.path.join(log_dir, "time.txt")

if os.path.exists(time_path):
    with open(time_path, "r") as f:
        training_time = float(f.read())

    print(f"Tiempo entrenamiento (min): {training_time/60:.2f}")
else:
    print("No time.txt encontrado")

# =========================
# ENTORNO CON GRABACIÓN
# =========================

ENV_NAME = "ALE/Pacman-v5"

video_folder = os.path.join("videos", exp_name)
os.makedirs(video_folder, exist_ok=True)

env = gym.make(ENV_NAME, render_mode="rgb_array")

env = RecordVideo(
    env,
    video_folder=video_folder,
    episode_trigger=lambda ep: True,  # graba todos los episodios
    name_prefix=exp_name
)

# =========================
# EVALUACIÓN DETERMINÍSTICA
# =========================

scores = []

print("\n===== STARTING EVALUATION =====")

for ep in range(EPISODES):

    obs, _ = env.reset()
    done = False
    total_reward = 0

    while not done:
        action, _ = model.predict(obs, deterministic=True)
        obs, reward, terminated, truncated, _ = env.step(action)
        done = terminated or truncated
        total_reward += reward

    print(f"Episodio {ep+1}: {total_reward}")
    scores.append(total_reward)

env.close()

# =========================
# RESULTADOS FINALES
# =========================

print("\n===== EVALUATION METRICS =====")
print(f"Episodes: {EPISODES}")
print(f"Mean: {np.mean(scores):.2f}")
print(f"Std: {np.std(scores):.2f}")
print(f"Max: {np.max(scores):.2f}")
print(f"Min: {np.min(scores):.2f}")

print("\nVideos guardados en:", video_folder)